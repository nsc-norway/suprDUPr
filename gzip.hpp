...
public:
    ...
    int state() { return state_; } // Marius hack
    void state(int v) { state_ = v; } // Marius hack
private:
    enum state_type {
        s_id1       = 1,
        s_id2       = s_id1 + 1,
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                ...
                int prev_state = header_.state();
                header_.process(c);
                // Marius: Work-around for old boost version
                if (prev_state == 7 && header_.state() == 9)
                    header_.state(8);
                if (header_.done())
                    state_ = s_body;
